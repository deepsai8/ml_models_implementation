...
class MF(nn.Module):
    ...
    def forward(self, u, v):
        ...
        # return torch.from_numpy(1 / (1 + np.exp(-((U*V).sum(1) +  b_u  + b_v).detach().numpy())))
        return ((U*V).sum(1) +  b_u  + b_v).sigmoid()
        ### END SOLUTION
...
def train_one_epoch(model, train_df, optimizer):
    ...
    train_loss.backward()
    optimizer.step()
    # test_loss = valid_metrics(model, train_df)
    ### END SOLUTION
...
def valid_metrics(model, valid_df):
    ...
    valid_acc = np.sum(np.around(y_hat.detach().numpy(),0) == np.around(ratings.detach().numpy(),0)) / valid_df.shape[0]
    ### END SOLUTION
