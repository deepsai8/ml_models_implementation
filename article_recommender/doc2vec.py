...
def load_articles(articles_dirname, gloves):
    ...
    """
    #fully-qualified list of filenames under root directory
    files = filelist(articles_dirname)

    output = []
    for i in files:
        path = i.split('bbc/')[-1]
        all_txt = get_text(i).split('\n\n')

        title = all_txt[0]
        txt = ''
        for j in all_txt[1:]:
            txt += j
        wvec = doc2vec(txt, gloves)
        output.append([path, title, txt, wvec])
...
def doc2vec(text, gloves):
    ...
    for w in wd:
        if w in gloves.keys():
            c += 1
            wvec += np.array(gloves[w]).astype(float)
    if c == 0:
        output = 0
    else:
        output = wvec/c
...
def recommended(article, articles, n):
    ...
    recos = sorted(dist,key=lambda dist:(dist[0]), reverse=False)
    output = []
    for i in recos[0:n]:
        output.append(i[1])
    return output